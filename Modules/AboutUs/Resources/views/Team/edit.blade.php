...
<script type="text/javascript">
    $(document).ready(function () {
        $('#teamEditForm').formValidation({
            ...
            $('#teamEditForm').formValidation('revalidateField', 'name');
            $('#teamEditForm').formValidation('revalidateField', 'post');
            $('#teamEditForm').formValidation('revalidateField', 'description');
        })
                ...
                .each(function () {
                    $(this)
                        ...
                        .on('change', function (e) {
                            $('#teamEditForm').formValidation('revalidateField', e.sender.name);
                        });
                });
    });
</script>
...
<div class="box box-primary">
    ...
    <!-- /.box-header -->
    <!-- form start -->
    <form id="teamEditForm" method="POST" action="{{ route('admin.about-us.team.update', $team->id) }}" enctype="multipart/form-data">
        {{ csrf_field() }}
        <div class="box-body">
        @include('layouts.backend.alert')
            <div class="form-group">
                <label for="name">Name *</label>
                <input type="text" name="name" class="form-control" id="name" placeholder="Enter name." value="{{ $team->name }}" >
            </div>

            <div class="form-group">
                    <label for="post">Post *</label>
                    <input type="text" name="post" class="form-control" id="post" placeholder="Enter post." value="{{ $team->post }}" >
                </div>
            ...
            <div class="form-group">
                ...
                <div class="fileupload fileupload-new" data-provides="fileupload">
                    <div class="fileupload-new thumbnail" >
                        ...
                        @else
                            <img src="{{ asset('uploads/aboutus/teams/default-img.jpg') }}" alt="default-image">
                        @endif
                    </div>
            ...
            <div class="form-group">
                ...
                <option value="1" {{ ($team->is_active == 1) ? 'selected=selected' : '' }}>Published</option>
                <option value="0" {{ ($team->is_active == 0) ? 'selected=selected' : '' }}>Unpublished</option>
                </select>
            </div>
