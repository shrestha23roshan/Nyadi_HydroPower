...
if (segments[8] === 'create') {
    type = 'POST';
    id = segments[6];
    url = "/admin/media-management/album/" + id + "/photo/store";
    removeLink = true;
} else if (segments[9] === 'edit') {
    type = 'PUT';
    id = segments[8];
    method = segments[9];
    url = baseUrl + "/admin/media-management/album/photos/" + id;
    removeLink = true;
} 
...
$("#mydropzone").dropzone({
    ...
    init: function () {
        ...
        this.on("successmultiple", function (files, response) {
            ...
            window.location.href = "/admin/media-management/album/" + id + "/photo";
        });
        ...
        this.on("removedfile", function (file) {
            if (segments[9] === 'edit') {
                $.post({
                    url: '/admin/media-management/album/photos/delete',
...
function getPhotos(myDropzone, id) {
    $.getJSON('/media-management/album/' + id + '/photos', function (response) { // get the json response
        $.each(response.data, function (key, item) { //loop through it
            let path = base_url + "uploads/media-management/album/" + item.name;
